...
db.getConnection((err, connection) => {
    ...
    connection.query(schema, (err) => {
        if (err) console.error('Error creando tablas:', err);
            ...
            // Forzar actualización de esquema para Tabla Orders (Agrega columnas si no existen)
            const addCols = [
            ...
            addCols.forEach(cmd => connection.query(cmd, () => {})); // Ignorar silenciosamente si ya existen
...
// 4. Crear una Orden (Modificado para Usuarios Invitados)
app.post('/api/orders', (req, res) => {
    const { items, total, shipping, customer, email, phone, address, notes } = req.body;
    const userId = null; // Clientes sin cuenta

    const query = `
        INSERT INTO orders 
        (user_id, total_amount, status, customer_name, email, phone, address, shipping_method, notes) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    db.query(query, [userId, total, 'pendiente', customer, email, phone, address, shipping, notes], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
